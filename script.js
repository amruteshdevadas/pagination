...
function buttons(pages)
{
    ...
    //     <button value ="${page}" class ="page btn btn-lg btn-info">${page} </button>`
    ...
    for (var page = maxLeft; page <= maxRight; page++) {
    	pageButtons.innerHTML += `<button value=${page} class="page btn btn-lg btn-sm btn-info">${page}</button>`
        document.body.appendChild(pageButtons)
    }

    if (state.page != 1) {
        pageButtons.innerHTML = `<button value=${1} class="page btn btn-lg btn-sm btn-info">&#171; Previous</button>` + pageButtons.innerHTML
        document.body.appendChild(pageButtons)
    }

    if (state.page != pages) {
        pageButtons.innerHTML += `<button value=${pages} class="page btn btn-lg btn-sm btn-info">Next &#187;</button>`
        document.body.appendChild(pageButtons)
    }
